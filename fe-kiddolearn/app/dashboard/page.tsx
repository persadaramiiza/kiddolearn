...
export default function DashboardPage() {
  ...
  const [siswaroomEmail, setSiswaroomEmail] = useState(''); // Email for linking profile to SiswaRoom
  ...
  const handleAddProfile = async (e: React.FormEvent) => {
    ...
    try {
      await profilesService.create({
        ...
        siswaroom_email: siswaroomEmail || undefined, // Send manual email if provided
      });
      setNewProfileName('');
      setNewProfileAge(5);
      setSiswaroomEmail('');
      ...
      const message = error.response?.data?.message || 'Gagal menambah profil';
      setError(message);
    }
  };
        ...
        <header className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6 sm:mb-12 bg-white/90 backdrop-blur-md border-b-2 border-[#FFE0B2] shadow-sm rounded-[2rem] sm:rounded-full px-4 py-3 sm:px-8 sm:py-4">
          ...
          <div className="flex items-center gap-4 w-full sm:w-auto justify-between sm:justify-end">
            ...
            <Button
              ...
              className="bg-[#8B7355] hover:bg-[#6B5845] text-white font-bold rounded-full px-3 py-1.5 sm:px-4 sm:py-2 flex items-center gap-2"
            >
            ...
            <Button
              ...
              className="bg-[#1565C0] hover:bg-[#0D47A1] text-white font-bold rounded-full px-3 py-1.5 sm:px-4 sm:py-2 flex items-center gap-2"
            >
              <span className="text-base sm:text-xl">📊</span>
                  ...
                  >
                    <div className="relative">
                      ...
                      {profile.siswaroom_user_id && (
                        <div className="absolute -top-1 -right-1 w-6 h-6 sm:w-8 sm:h-8 bg-[#1565C0] rounded-full flex items-center justify-center text-white text-[10px] sm:text-xs shadow-lg border-2 border-white" title="Terhubung ke SiswaRoom">
                          🌐
                        </div>
                      )}
              ...
              <form onSubmit={handleAddProfile} className="space-y-6">
                ...
                <div className="space-y-2">
                  <label className="block text-sm font-black text-[#4A4A4A]">
                    Hubungkan dengan SiswaRoom (Opsional)
                  </label>
                  <div className="space-y-3">
                    <input
                      type="email"
                      value={siswaroomEmail}
                      onChange={(e) => setSiswaroomEmail(e.target.value)}
                      placeholder="Email SiswaRoom (untuk validasi ketat)"
                      className="w-full px-4 py-3 bg-[#F0F7FF] border-2 border-[#B2D7FF] rounded-xl text-[#1565C0] placeholder-[#1565C0]/50 focus:outline-none focus:border-[#1565C0] font-bold transition-colors"
                    />
                    <p className="text-[10px] text-[#8B7355] font-medium italic">
                      * Jika email tidak terdaftar di SiswaRoom, profil tidak akan terbuat.
                    </p>
                  </div>
                </div>
