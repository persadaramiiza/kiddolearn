...
import { siswaroomService, SiswaroomVideoItem } from '@/lib/siswaroom';
...
export default function ProfileDashboardPage() {
  ...
  const [siswaroomVideos, setSiswaroomVideos] = useState<SiswaroomVideoItem[]>([]);
  const [loadingVideos, setLoadingVideos] = useState(false);
  const [loadingSiswaroom, setLoadingSiswaroom] = useState(false);
  ...
  const loadProfileAndVideos = useCallback(async () => {
    ...
    try {
      ...
      // Load SiswaRoom Videos if profile is linked
      if (profileData.siswaroom_user_id) {
        setLoadingSiswaroom(true);
        try {
          const accountsWithVideos = await siswaroomService.getAllVideos();
          // Filter videos for this specific profile's siswaroom_user_id
          const profileAccount = accountsWithVideos.find(
            acc => acc.siswaroom_user_id === profileData.siswaroom_user_id
          );
          
          if (profileAccount) {
            setSiswaroomVideos(profileAccount.videos);
          }
        } catch (srErr) {
          console.error('Error loading SiswaRoom videos:', srErr);
        } finally {
          setLoadingSiswaroom(false);
        }
      }
    } catch (err: any) {
  ...
  return (
    <div className="min-h-screen bg-[#FFF9F0] font-sans text-[#4A4A4A]">
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {videos.map((video, index) => (
              ...
              >
                ...
                <div className="p-4">
                  <h3 className="font-black text-[#4A4A4A] line-clamp-2 group-hover:text-[#FF7A00] transition-colors">
                    {video.title}
                  </h3>
                  <p className="text-sm text-[#8B7355] mt-2 line-clamp-2 font-bold">
                    {video.description}
                  </p>
                  <div className="flex items-center justify-between mt-4">
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-black text-[#FF7A00] bg-[#FFF5E5] px-2 py-1 rounded-full">
                        {video.category || 'Edukasi'}
                      </span>
                    </div>
                    <div className="text-xs font-bold text-[#8B7355]">
                      {video.duration_minutes ? `${video.duration_minutes} menit` : ''}
                    </div>
                  </div>
                </div>
              </div>
        ...
        {/* SiswaRoom Videos Section */}
        {profile.siswaroom_user_id && (
          <div className="mt-16">
            <div className="flex items-center gap-3 mb-8">
              <div className="w-12 h-12 bg-[#1565C0] rounded-2xl flex items-center justify-center text-2xl shadow-lg">
                🌐
              </div>
              <div>
                <h2 className="text-2xl sm:text-3xl font-black text-[#4A4A4A]">
                ...
                <p className="text-[#8B7355] font-bold">Materi belajar dari sekolahmu</p>
              </div>
            </div>
            ...
            {loadingSiswaroom ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {[...Array(3)].map((_, i) => (
                  <div key={i} className="bg-white rounded-2xl h-32 animate-pulse border-2 border-[#E3F2FD]"></div>
                ))}
              </div>
            ) : siswaroomVideos.length === 0 ? (
              <div className="bg-white rounded-3xl p-10 text-center border-2 border-dashed border-[#B2D7FF]">
                <div className="text-5xl mb-4">📖</div>
                <h3 className="text-lg font-black text-[#4A4A4A]">Belum Ada Materi</h3>
                <p className="text-[#8B7355] font-bold">Belum ada video materi dari SiswaRoom untuk akun ini.</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {siswaroomVideos.map((video, index) => (
                  <motion.div
                    key={video.id}
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ delay: index * 0.1 }}
                    className="bg-white rounded-2xl p-4 shadow-md border-b-4 border-[#1565C0] hover:-translate-y-1 transition-all cursor-pointer group"
                    onClick={() => {
                      router.push(`/watch/sr-${video.id}?profile=${profile.id}&source=siswaroom&url=${encodeURIComponent(video.video_url)}&title=${encodeURIComponent(video.material_title)}`);
                    }}
                  >
                    <div className="flex gap-4">
                      <div className="w-16 h-16 bg-[#E3F2FD] rounded-xl flex-shrink-0 flex items-center justify-center text-3xl group-hover:scale-110 transition-transform">
                        🎬
                      </div>
                      <div className="flex-1 min-w-0">
                        <h4 className="font-black text-[#4A4A4A] text-sm line-clamp-2 group-hover:text-[#1565C0] transition-colors">
                          {video.material_title}
                        </h4>
                        <p className="text-xs text-[#8B7355] font-bold mt-1 truncate">
                          {video.course_title}
                        </p>
                        {video.duration_minutes && (
                          <div className="flex items-center gap-1 mt-2 text-[10px] font-black text-[#1565C0] bg-[#E3F2FD] w-fit px-2 py-0.5 rounded-full">
                            <span>⏱️</span>
