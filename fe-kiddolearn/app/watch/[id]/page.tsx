...
import { ArrowLeft, Eye, Star, CheckCircle2, AlertCircle, PlayCircle, Clock, Brain, ExternalLink, Download } from "lucide-react";
...
function WatchPage() {
  ...
  const srUrl = searchParams?.get('url');
  const srTitle = searchParams?.get('title');
  
  const isSiswaRoom = source === 'siswaroom' || rawVideoId?.startsWith('sr-');
  const isPlaceholder = srUrl?.includes('example.com');
  const videoId = isSiswaRoom ? 0 : Number(rawVideoId);
  ...
  const loadVideo = useCallback(async () => {
    try {
      ...
      if (isSiswaRoom) {
        if (!srUrl) {
          setError('URL video SiswaRoom tidak ditemukan');
          return;
        }

        const isPdf = srUrl.toLowerCase().endsWith('.pdf');
        const srVideoData: Video = {
          id: 0,
          title: srTitle || 'Video SiswaRoom',
          description: 'Konten dari SiswaRoom',
          video_url: srUrl,
          platform: isPdf ? 'native' : (srUrl.includes('youtube.com') || srUrl.includes('youtu.be') ? 'youtube' : 'native'),
          category: 'SiswaRoom',
          min_age: 0,
          max_age: 18,
          view_count: 0,
          status: 'published',
          creator_id: 0,
          creator: { id: 0, full_name: 'SiswaRoom', email: 'info@siswaroom.online' },
          created_at: new Date().toISOString(),
        };

        setVideo(srVideoData);
  ...
  }, [videoId, profileId, isSiswaRoom, srUrl, srTitle]);

  // ==================== LOAD QUIZZES ====================
  const loadQuizzes = useCallback(async () => {
    if (!videoId || isSiswaRoom) return;
    ...
    const initPlayer = () => {
      ...
      const checkAndInit = () => {
        ...
        try {
              ...
              onError: (event: any) => {
                ...
                switch (event.data) {
                  case 2:
                    ...
                    break;
                  case 5:
                    errorMessage = 'Error HTML5 player.';
                    break;
                  case 100:
                  ...
                  case 101:
                  case 150:
                    errorMessage = 'Video tidak dapat disematkan karena pembatasan dari pemilik video.';
                    break;
                  default:
                    errorMessage = `Error YouTube player (kode: ${event.data})`;
                }
          ...
          setError('Gagal membuat YouTube player. Video mungkin tidak dapat disematkan.');
          setPlayerReady(true);
  ...
  useEffect(() => {
    if ((videoId || isSiswaRoom) && user && !authLoading) {
      loadVideo();
      loadQuizzes();
    }
  }, [videoId, isSiswaRoom, user, authLoading, loadVideo, loadQuizzes]);
      ...
      <main className="container mx-auto px-4 py-6 md:py-8 max-w-6xl">
        <div className="relative bg-black rounded-[1.5rem] md:rounded-[2.5rem] overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.2)] border-[6px] md:border-[8px] border-[#FFE0B2]">
          {isYouTube && youtubeId ? (
              ...
              {error && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/90 z-20">
                  <div className="text-white text-center max-w-md px-6">
                    ...
                    <h3 className="text-xl font-black mb-2">Video Tidak Dapat Dimuat</h3>
                    ...
                    <div className="space-y-3">
                      <a
                        ...
                        className="inline-flex items-center gap-2 px-6 py-2 bg-[#FF7A00] text-white rounded-full font-black hover:bg-[#E66E00] transition-all shadow-md"
                      >
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-2 px-6 py-2 bg-gray-600 text-white rounded-full font-black hover:bg-gray-500 transition-all shadow-md"
                      >
                        🔄 Coba Lagi
                      </button>
                    </div>
                    <div className="text-sm text-gray-400 mt-4">
                      <p>Coba refresh halaman atau hubungi admin jika masalah berlanjut.</p>
                    </div>
                  </div>
                </div>
              )}
            ...
            video.video_url.toLowerCase().endsWith('.pdf') ? (
              <div className="flex flex-col w-full">
                <iframe
                  ...
                  title={video.title}
                />
                <div className="bg-white p-4 border-t flex flex-wrap justify-center gap-4">
                  <a
                    href={video.video_url}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 px-6 py-2 bg-[#FF7A00] text-white rounded-full font-black hover:bg-[#E66E00] transition-all shadow-md"
                  >
                    <ExternalLink className="h-4 w-4" />
                  ...
                  <a
                    href={video.video_url}
                    download
                    className="flex items-center gap-2 px-6 py-2 bg-[#FFF5E5] text-[#FF7A00] border-2 border-[#FF7A00] rounded-full font-black hover:bg-[#FFE0B2] transition-all shadow-md"
                  >
        ...
        <div className="mt-8 bg-white rounded-[2rem] p-6 md:p-8 shadow-xl border-4 border-white ring-4 ring-[#FF7A00]/10">
          {isPlaceholder && (
            <div className="mb-6 p-4 bg-amber-50 border-2 border-amber-200 rounded-2xl flex items-start gap-3">
              ...
              <div>
                <p className="font-black text-amber-800">Mode Demo / Placeholder</p>
                <p className="text-sm text-amber-700 font-bold">
                  Materi ini menggunakan URL contoh (example.com) dari SiswaRoom. 
                  Pastikan materi di SiswaRoom sudah memiliki file PDF atau video yang valid.
                </p>
              </div>
            </div>
          )}
