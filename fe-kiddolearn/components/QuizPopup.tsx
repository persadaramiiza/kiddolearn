...
export default function QuizPopup({ quiz, onSubmit, onClose, profileId }: QuizPopupProps) {
  ...
  const handleAskAI = async () => {
    ...
    try {
      ...
      // Create context from quiz question (without giving away the answer)
      const topic = quiz.question_text;
      const response = await aiService.getQuizHint(topic, quiz.options.map(o => o.option_text));
      setAiExplanation(response.response);
    } catch (err: any) {
      console.error('Failed to get AI explanation', err);
      setAiExplanation('Maaf, AI sedang tidak tersedia. Coba lagi nanti ya!');
    } finally {
      setAiLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-[2rem] max-w-lg w-full p-8 shadow-2xl border-4 border-[#FFE0B2] animate-scale-in">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <button
            onClick={handleAskAI}
            disabled={aiLoading}
            className="px-4 py-2 bg-gradient-to-r from-purple-500 to-indigo-500 text-white rounded-xl font-bold hover:from-purple-600 hover:to-indigo-600 flex items-center gap-2 shadow-md"
          >
            {aiLoading ? (
              <>
          ...
          <div className="mb-4 p-4 bg-gradient-to-r from-purple-50 to-indigo-50 rounded-xl border border-purple-200">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-purple-700">Penjelasan AI</span>
              </div>
              <button
